...
export default function SellerCashClosures() {
  const title = 'Fecho de Caixa';
  const queryClient = useQueryClient();
  const [limit, setLimit] = useState(50);

  const { data: cashClosures = [], isLoading } = useQuery({
    queryKey: ['seller-cash-closures', limit],
    queryFn: () => base44.staff.cashClosures.list('-created_date', limit),
    staleTime: 60_000,
  });
  ...
  const [cashClosureForm, setCashClosureForm] = useState({
    started_at: formatLocalYmd(new Date()),
    ended_at: formatLocalYmd(new Date()),
    opening_balance: '',
    notes: '',
  });

  const { data: salesSummary } = useQuery({
    queryKey: ['seller-cash-closures-summary', cashClosureForm.started_at, cashClosureForm.ended_at],
    queryFn: () => base44.staff.cashClosures.summary({ started_at: cashClosureForm.started_at, ended_at: cashClosureForm.ended_at }),
  ...
  const handleCreateCashClosure = async (event) => {
    ...
    try {
      await base44.staff.cashClosures.create({
      ...
      setCashClosureForm({ started_at: formatLocalYmd(new Date()), ended_at: formatLocalYmd(new Date()), opening_balance: '', notes: '' });
      queryClient.invalidateQueries({ queryKey: ['seller-cash-closures'] });
    } catch (err) {
      console.error(err);
  ...
  const exportCsv = () => {
    const now = formatLocalYmd(new Date());
    const rows = [
          ...
          <form onSubmit={handleCreateCashClosure} className="space-y-4">
            ...
            <div className="grid gap-3 md:grid-cols-2">
              ...
              <div>
                ...
                <Input value={moneyPt(closingBalanceNumber)} placeholder="Saldo final" className="rounded-none mt-1" disabled />
              </div>
            </div>
            <div>
              <label className="font-body text-xs">Total de vendas</label>
              <Input value={moneyPt(totalSalesNumber)} placeholder="Total de vendas" className="rounded-none mt-1" disabled />
            </div>
            <textarea
